...
def connect_one(ip: str, port: int, timeout: float):
    ...
    try:
        ...
        ###########################################################
        # 연결 직전(start)과 직후(end)의 시간 차이를 ms 단위로 계산합니다.
        s.connect((ip, port))
...
def connect_with_fallback(ips: list[str], port: int, timeout: float, prefer: str = "any") -> TCPConnectResult:
    ...
    # 1. prefer 정책에 따라 v4, v6 주소의 우선순위가 반영된 ordered 리스트를 만듭니다.
    v4 = [ip for ip in ips if ":" not in ip]  # IPv4: ':' 없음
    v6 = [ip for ip in ips if ":" in ip]      # IPv6: ':' 있음
    ...
    for ip in ordered:
        # 2. connect_one으로 연결을 시도하고, 성공 시 정보를 추출하여 반환합니다.
